...
public class FixedTableDraw extends Canvas
{
		...
		addMouseMoveListener(new MouseMoveListener() 
			{
				...
				{
					if (e==null || offset==null || fontwidth==0) return;
	...
	private FixedFileInputField getFieldOnPosition(int x)
	{
		...
		for(int i=0;i<fields.size();i++)
		{
			...
			if (position<=x && position+width>x) return field;
			position+=width;
		}
		return null;
	}
	...
	private void setMarker(int x)
	{
		...
		for (int i=0;i<fields.size() && idx<0;i++)
		{
			...
			if ( highest_smaller<0 && position+width >= x) highest_smaller=i; // The first time this occurs is OK.
